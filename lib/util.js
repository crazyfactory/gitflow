...
function toIssueNumberAndName(issue, prefix = '') {
  if (prefix) {
    issue = issue.substring(prefix.length);
  }
  const arr = issue.split('-');
  const issueNumber = arr[0];
  if (isNaN(issueNumber)) {
    throw new Error('Your branch name is not in the format: {issue_number}-{feature_name}')
  }
