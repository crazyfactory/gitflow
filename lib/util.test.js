...
describe('util', () => {
  ...
  describe('toIssueNumberAndName', () => {
    ...
    it('throws if input is not in correct format', () => {
      ...
      }).toThrow('Your branch name: "abc" is not in the format: {issue_number}-{feature_name}');

      expect(() => {
        toIssueNumberAndName('a-feature-a');
      }).toThrow('Your branch name: "a-feature-a" is not in the format: {issue_number}-{feature_name}');
    })
  });
