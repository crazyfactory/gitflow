...
async function createPull({owner, repo, title, head, base, body}) {
  return await octokit.pulls.create({owner, repo, title, head, base, body});
}
...
async function generateToken() {
  ...
  octokit = new Octokit({
    auth: {
      ...credentials,
      ...(credentials['2FAEnabled'] && { on2fa: inquirer.askGithub2FactorAuthenticationCode })
    },
  });
